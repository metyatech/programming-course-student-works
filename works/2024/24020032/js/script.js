...
function setFadeElement(){
    ...
    //出現範囲の指定
	var contentsTop = Math.round($('#Access').offset().top);	//要素までの高さを取得
	var contentsH = $('#Access').outerHeight(true);	//要素の高さを取得
...
$(window).scroll(function () {
	setFadeElement();// 指定の範囲内で下から出現の関数を呼ぶ
    fadeAnime();// 最低限おぼえておきたい動きの関数を呼ぶ
    slideAnime();//  テキストが流れるように出現（左から右）の関数を呼ぶ
	GlowAnimeControl();// テキストがほのかに光りながら出現の関数を呼ぶ
	BlurTextAnimeControl();// テキストがじわっと出現の関数を呼ぶ
});
